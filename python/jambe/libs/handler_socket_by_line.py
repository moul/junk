...
class jambeHandlerSocketByLine(SocketServer.StreamRequestHandler):
    __base = SocketServer.StreamRequestHandler
    ...
    def __init__(self, request, client_address, server):
        ...
        self._jambe.Log.log('handler socket by line init')
        self._jambe.Mh.invoke_all('HandlerSocketByLine_init', self)
        SocketServer.StreamRequestHandler.__init__(self, request, client_address, server)

    def setup(self):
        SocketServer.StreamRequestHandler.setup(self)
        self._jambe.Mh.invoke_all('HandlerSocketByLine_setup', self)

    def handle(self):
        self._jambe.Mh.invoke_all('HandlerSocketByLine_handle', self)
    ...
    def handle_one_request(self):
        line = self._readline()
        self._jambe.Log.log('line: "%s"' % line.strip())
        if line[-1:] != '\n':
            result = self._jambe.Mh.invoke_all('HandlerSocketByLine_client_disconnected', self)
            self.close_connection = 1
            return
        self._jambe.Mh.invoke_all('HandlerSocketByLine_handle_one_request', self, line)

    def _readline(self):
        self._lastline = self.rfile.readline()
        self._jambe.Log.log('<<< %s' % self._lastline.strip())
        return self._lastline

    def _write(self, content):
        self._jambe.Log.log('>>> %s' % content.strip())
...
def setup(jambe):
    return jambeHandlerSocketByLine(jambe)

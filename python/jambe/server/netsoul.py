...
class jambeProxy(jambe.Jambe):
    ...
    def _bootstrap_netsoul(self):
        import jambe.libs.thread
        import jambe.libs.handler_socket_by_line
        jambe.libs.handler_socket_by_line.jambeHandlerSocketByLine.__version__ = self.version()
        ...
        import socket
        try:
            self.Proxy = jambe.libs.thread.setup(self, jambe.libs.handler_socket_by_line.jambeHandlerSocketByLine, (server_address, server_port))
...
def main():
    ...
    jambe.variable_set('enabled_modules', ['core', 'netsoul'])
#, 'admin', 'user', 'session'])
    try:
        jambe.bootstrap(jambe.BOOT_FULL)
        jambe._bootstrap_netsoul()
